...
# --- Build the Stockfish 17 engine ---
# `make build` downloads the default network and embeds it into the binary, so
# the engine is self-contained. ARCH=x86-64-avx2 is safe on all modern cloud
# CPUs and gives fast NNUE inference.
WORKDIR /app/engine
COPY . /app/engine
RUN git clone --depth 1 -b sf_17 https://github.com/official-stockfish/Stockfish.git stockfish_src \
 && cd stockfish_src/src \
 && make -j build ARCH=x86-64-avx2 COMP=gcc \
 && cp stockfish ../../stockfish \
 && cd ../../ \
 && ./stockfish bench 2>&1 | tail -4        # self-test: prints "Nodes searched"
